   ...
   CartItems.map((item) => (
      <div key={item.id} className='flex justify-between flex-row' >
          <div className='mt-5 flex md:gap-10 gap-5 ' >
            <img className='object-cover md:w-48 md:h-48 w-24 h-24 ' src={item.url}/>
            <div className='flex flex-col gap-2'>
                ...
                <div onClick={ ()=>{handleRemoveItem(item.id)} }  className='hover:bg-slate-500 p-1 rounded-full' >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
      ...
      <button onClick={handleCheckout}  className="bg-black px-8 py-5 w-full  text-white rounded-full font-bold hover:bg-gray-600">Proceed To Checkout</button>
